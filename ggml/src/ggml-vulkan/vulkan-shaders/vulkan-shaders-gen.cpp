...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        ...
        // Fused MUL_MAT(gate) + MUL_MAT(up) + SWIGLU_SPLIT (Stage 2: Q4_K only)
        if (tname == "q4_k") {
            const std::string glu_src = "mul_mat_vec_" + tname + "_glu.comp";
            string_to_spv("mul_mat_vec_" + tname + "_glu_f32_f32",                   glu_src, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float"}, {"B_TYPEV2", "vec2"}, {"B_TYPEV4", "vec4"}, {"D_TYPE", "float"}}));
            string_to_spv("mul_mat_vec_" + tname + "_glu_f32_f32_subgroup",          glu_src, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float"}, {"B_TYPEV2", "vec2"}, {"B_TYPEV4", "vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD", "1"}}));
            string_to_spv("mul_mat_vec_" + tname + "_glu_f32_f32_subgroup_no_shmem", glu_src, merge_maps(base_dict, {{data_a_key, "1"}, {"B_TYPE", "float"}, {"B_TYPEV2", "vec2"}, {"B_TYPEV4", "vec4"}, {"D_TYPE", "float"}, {"USE_SUBGROUP_ADD_NO_SHMEM", "1"}}));
        }
...
void write_output_files() {
    ...
    for (const auto& tname : type_names) {
        ...
        // Fused gate+up+SWIGLU_SPLIT aggregation (Stage 2: Q4_K only, f32 B)
        if (tname == "q4_k" && btype == "f32") {
            hdr << "extern const void * arr_dmmv_" << tname << "_glu_" << btype << "_f32_data[3];\n";
            hdr << "extern const uint64_t arr_dmmv_" << tname << "_glu_" << btype << "_f32_len[3];\n";
            if (basename(input_filepath) == "mul_mat_vec_" + tname + "_glu.comp") {
                src << "const void * arr_dmmv_" << tname << "_glu_" << btype << "_f32_data[3] = {mul_mat_vec_" << tname << "_glu_" << btype << "_f32_data, mul_mat_vec_" << tname << "_glu_" << btype << "_f32_subgroup_data, mul_mat_vec_" << tname << "_glu_" << btype << "_f32_subgroup_no_shmem_data};\n";
                src << "const uint64_t arr_dmmv_" << tname << "_glu_" << btype << "_f32_len[3] =  {mul_mat_vec_" << tname << "_glu_" << btype << "_f32_len,  mul_mat_vec_" << tname << "_glu_" << btype << "_f32_subgroup_len, mul_mat_vec_"  << tname << "_glu_" << btype << "_f32_subgroup_no_shmem_len};\n";
            }
        }
    }
    }
